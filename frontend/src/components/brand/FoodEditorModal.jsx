...
export default function FoodEditorModal({
      ...
      for (const z of zones) {
        ...
        priceMap[z.id] = existing
          ? {
              ...
              discount_active: existing.discount_active,
            }
  ...
  const save = async (e) => {
    ...
    try {
      ...
      const savedFood = isEditing
        ? await foodsApi.update(brandId, food.id, payload)
        : await foodsApi.create(brandId, payload);
      ...
      <form onSubmit={save} className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Textarea
            ...
            onChange={(e) =>
              setForm({ ...form, description_kh: e.target.value })
            }
            className="font-khmer"
          />
        </div>
        ...
        <div>
          ...
          {zones.length === 0 ? (
            <p className="text-sm text-slate">
              Add a zone to this brand first before setting prices.
            </p>
          ) : (
                ...
                <div key={z.id} className="rounded-md border border-sand p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        updatePrice(z.id, "regular_price", e.target.value)
                      }
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        updatePrice(z.id, "discount_price", e.target.value)
                      }
                    />
                  </div>
                  <div className="mt-2">
                    <Checkbox
                      ...
                      onChange={(e) =>
                        updatePrice(z.id, "discount_active", e.target.checked)
                      }
                    />
                  </div>
                </div>
